...
data_vector = net.get_train_data()[0][index:index + 1]
data_label = net.get_train_data()[1][index:index + 1]

predict = net.predict(200, data_vector)

data_for_graph = []

for i in range(len(data_vector[0])):
    data_for_graph.append(1 - data_vector[0][i][0])

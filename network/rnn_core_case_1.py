...
class RNNCoreCase1_1(RNNCoreCase1):
    ...
    def read_data(self):
            ...
            for i in range(len(lines)):
                ...
                if index >= 0:
                    ...
                    if answer < len(data_vector):
                        ...
                        data_all[1].append(zeros)
                        exist = False
                        for data_origin in data_all[3]:
                            if data_origin['origin_file'] == data_split[0] and data_origin['index'] == index:
                                exist = True
                                break

                        data_all[3].append({'origin_file': data_split[0], 'max_value': max_value[0], 'index': index, 'isRight': exist})
        ...
        ], [tf.convert_to_tensor(data_all[1][:sp], dtype=tf.float32)], data_all[3][:sp])
